...
class RequestSubstate extends MusicBeatSubstate {
	...
	public static function requestURL(url:String, ?prompt:String = "是否打开此链接", ?disableTrusting:Bool = false) {
	...
	public static function requestDownload(url:String, ?prompt:String = "是否下载此文件", ?onDownloadFinished:String->Void, ?disableTrusting:Bool = false, ?yesCallback:Void->Void) {
		request(prompt, url, nowTrusting -> {
			if (nowTrusting != null) {
				var splitURL = url.split("//");
				if (nowTrusting)
	...
	override function create() {
		...
		if (!disableTrusting) {
			add(trust = new Option("信任此来源", "勾选后，以后访问该域名的链接\n将不再询问。", () -> {
